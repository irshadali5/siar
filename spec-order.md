...
| **1.3** | [`03-transport-routing-policy-engine-architecture.md`](sys-arch/03-transport-routing-policy-engine-architecture.md) | **200** | **92/200 (46%)** | [`siar-routing-policy`](crates/siar-routing-policy), [`siar-connectivity`](crates/siar-connectivity) | **ACTIVE FOCUS**: Dynamic link scoring, path diversity (underlay), quality signals. Next: §75+ |
...
| 03 | [Transport & Routing Policy](sys-arch/03-transport-routing-policy-engine-architecture.md) | 200 | 92/200 (46%) | Milestone 1 (Active) |
...
Current Status   : 92 / 200 Sections (46% Reconciled, 82/82 Tests Passing)
```
...
2. **Next Batch (Round 6)**: Implement sections **§75 onward** of Spec 03:
   * Multipath Chunk Scheduler (§75)
   * File Transfer Retry & Resumable State (§76–§77)
   * Route Planning for Calls & Call Path Selection (§78–§81)
   * Platform Power & Battery Level Awareness (§82–§84)
