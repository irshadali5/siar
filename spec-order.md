...
| **1.3** | [`03-transport-routing-policy-engine-architecture.md`](sys-arch/03-transport-routing-policy-engine-architecture.md) | **200** | **101/200 (51%)** | [`siar-routing-policy`](crates/siar-routing-policy), [`siar-connectivity`](crates/siar-connectivity) | **ACTIVE FOCUS**: Congestion signals, stability score, metered/roaming tri-state. Next: §85+ |
...
| 03 | [Transport & Routing Policy](sys-arch/03-transport-routing-policy-engine-architecture.md) | 200 | 101/200 (51%) | Milestone 1 (Active) |
...
Current Status   : 101 / 200 Sections (51% Reconciled, 95/95 Tests Passing)
```
...
2. **Next Batch (Round 7)**: Implement sections **§85 onward** of Spec 03:
   * Battery-Aware Inputs & BatteryLevelClass/DeviceState (§85)
   * Background Restrictions & Foreground Isolation (§86)
   * Platform Policy Integration & Radio Availability (§87)
   * Discovery Budget & Throttling (§88–§90)
