...
pub fn resolve_destination_devices(destination: Destination, trust_store: &TrustedAccountStore) -> Result<Vec<DeviceId>, RoutingError> {
    match destination {
        Destination::Device(device_id) => Ok(vec![device_id]),
        Destination::Account(account_id) => {
            let directory = trust_store.directory_for(account_id).ok_or(RoutingError::NoActiveDevicesForAccount)?;
            let devices: Vec<DeviceId> = directory.active_devices().map(|entry| entry.device_id).collect();
            if devices.is_empty() {
...
mod tests {
    ...
    use siar_identity_multidevice::{
        DeviceCapabilitySet, DeviceCertificate, DeviceDirectory, DeviceDirectoryEntry, DeviceStatus, RootIdentityKey,
    };
    ...
    fn an_account_destination_resolves_to_its_active_devices_only() {
        ...
        trust_store.accept(directory, &root.root_public_key()).unwrap();

        let devices = resolve_destination_devices(Destination::Account(account), &trust_store).unwrap();
    ...
    fn an_account_with_no_trusted_directory_is_a_real_error_not_an_empty_list() {
        ...
        let result = resolve_destination_devices(Destination::Account(AccountId::new()), &trust_store);
    ...
    fn a_device_destination_resolves_to_itself_without_touching_the_trust_store() {
        ...
        let devices = resolve_destination_devices(Destination::Device(device), &trust_store).unwrap();
