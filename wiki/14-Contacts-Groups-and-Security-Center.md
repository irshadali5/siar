...
> **Key Crates:** [`crates/siar-identity-multidevice`](../crates/siar-identity-multidevice), [`crates/siar-crypto-mls`](../crates/siar-crypto-mls)
...
## 3. Security Center & Device Management

The **Security Center** interface gives users total sovereignty over their cryptographic perimeter:
...
When a user suspects a secondary device has been compromised:
1. The user taps **Emergency Lockdown** on their primary device.
2. The root key signs a global `RevocationTombstone` for all secondary `DeviceId`s.
3. The tombstone is broadcast across all mesh and relay links at **Priority 0**.
4. The compromised devices are instantly ejected from all MLS groups and disconnected.
